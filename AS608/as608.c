...
static u8 *JudgeStr(u16 waittime)
{
	...
	while(--waittime)
	{
		...
		TIM4_Delayms(1);
		if(USART1_GetRxFlag())//接收到一次数据
		{
			data=strstr((const char*)USART1_RxPacket,(const char*)str);
			if(data)
				return (u8*)data;	
		}
	}
...
//模块返回确认字
u8 PS_GetImage(void)//发送命令包，接收应答包
{
	u16 temp;
	u8  ensure;
	u8  *data;
...
//模块返回确认字
u8 PS_Match(u16 *score)//参数用来接收得分
{
	u16 temp;
	u8  ensure;
	u8  *data;
...
u8 PS_ReadSysPara(SysPara *p)
{
  ...
  u8  ensure;
	...
	if(data)
	{
		...
		p->PS_addr=(data[18]<<24)+(data[19]<<16)+(data[20]<<8)+data[21];	//2字32位设备地址
		p->PS_size = data[23];							//数据包大小
		p->PS_N = data[25];									//波特率设置
	}		
	else
		ensure=0xff;
	if(ensure==0x00)
...
//功能： 模块内部为用户开辟了512bytes的FLASH空间用于存用户记事本,
//	该记事本逻辑上被分成 16 个页。
...
u8 PS_HandShake(u32 *PS_Addr)
{
	...
	MYUSART_SendData(0X00);	
	TIM4_Delayms(200);
	if(USART1_GetRxFlag())//接收到数据
	{		
		
		if(//判断是不是模块返回的应答包				
					USART1_RxPacket[0]==0XEF&&USART1_RxPacket[1]==0X01&&USART1_RxPacket[6]==0X07
			)
			{
				*PS_Addr=(USART1_RxPacket[2]<<24) + (USART1_RxPacket[3]<<16)
								+(USART1_RxPacket[4]<<8) + (USART1_RxPacket[5]);
				return 0;
			}				
	}
	return 1;		
